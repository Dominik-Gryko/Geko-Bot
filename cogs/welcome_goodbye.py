...
class welcome_goodbye(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        welcome_embed = discord.Embed(title = f"Welcome to {member.guild.name}!", description= f"Welcome to the server! {member.mention} you are member {member.guild.member_count}!", color=discord.Color.teal())
        ...
        if data[str(member.guild.id)]["AutoRole"] is not None:
    ...
    async def on_member_remove(self, member):
        with open("./cogs/json/goodbye.json", "r") as f:
            data = json.load(f)
        
        goodbye_message = f"<@{str(member.id)}> just left the server!"
        goodbye_channel = discord.utils.get(member.guild.channels, name = data[str(member.guild.id)]["Channel"])
    ...
    async def goodbye(self, ctx):
        ...
        info_embed = discord.Embed(title="Goodbye System Setup", description="Create a simple goodbye system for your server!", color = discord.Color.teal())
        info_embed.add_field(name = "channel", value="Set a channel for your goodbye message to be sent in.", inline=False)
    ...
    async def channel(self, ctx, channel:discord.TextChannel):
        ...
        with open("./cogs/json/goodbye.json", "w") as f:
...
async def setup(bot):
    await bot.add_cog(welcome_goodbye(bot))
